// import React, {useEffect, useRef, useState} from 'react';
// import {
//   View,
//   StyleSheet,
//   Animated,
//   Pressable,
//   PanResponder,
//   Dimensions,
//   Image,
//   GestureResponderEvent
// } from 'react-native';
// import COLORS from '../constants/Colors';
// // import Feather from 'react-native-vector-icons/Feather'
// import {GRAPHIK_FONT, IMAGES} from '../constants/Constant';

// const IS_NATIVE_DRIVER = false;
// const SlideButton = ({
//   onSlide,
//   onSlideEnd,
//   onSlideRelease,
//   onSlideBegin,
//   onSlideConfirmed,
//   onSlideNotConfirmed,
//   defaultColor = COLORS.primary_pink,
//   defaultIconSize = 30,
//   tipAnimationEnable = false,
//   tipTextSlideAnimEnable = true,
//   sliderTipDuration = 1000,
//   sliderTipDistanceFromLeft = 40,
//   goToBackDuration = 1000,
//   confirmedPercent = 0.8,
//   ballPadding = 0,
//   disableOnConfirmed = false,
//   state = false,
//   unconfimredTipText = 'Accept Order',
//   confirmedTipText = 'Accept',
//   confirmedTipTextStyle = {},
//   unconfirmedTipTextStyle = {},
//   sliderButtonComponent = null,
//   sliderStyle,
// }) => {
//   const pan = useRef(new Animated.Value(0)).current;
//   const textAnim = useRef(new Animated.Value(0)).current;
//   const [disable, setDisable] = useState(false);
//   const [startPoint, setStartPoint] = useState(null);
//   const sliderWidth =
//     sliderStyle?.width || Dimensions.get('window').width * 0.8;

//   const getSliderWidth = () => {
//     const parentWidth = sliderButtonComponent?.props?.style?.width;
//     const childWidth =
//       sliderButtonComponent?.props?.children?.props?.style?.width;
//     if (parentWidth) {
//       return parentWidth;
//     } else if (childWidth) {
//       return childWidth;
//     } else {
//       return 40;
//     }
//   };

//   const sliderBallWidth = getSliderWidth();

//   const onMove = (_, gestureState) => {
//     if (gestureState.dx < 0 || gestureState.dx > scrollDistance) {
//         return Animated.event([{ dx: translateX }], { useNativeDriver: false })({
//             ...gestureState,
//             dx: gestureState.dx < 0 ? 0 : scrollDistance,
//         });
//     }
    
//     return Animated.event([{ dx: translateX }], { useNativeDriver: false })(gestureState);
// };

//   const panResponder = PanResponder.create({
//     onMoveShouldSetPanResponder: () => !disable,
//     onPanResponderGrant: e => {
//       onSlideBegin?.(e);
//       setStartPoint(e.nativeEvent.pageX);
//     },
//     onPanResponderMove: e => {
//       onSlide?.(e);
//       pan.setValue(e.nativeEvent.pageX - startPoint);
//     },
//     onPanResponderRelease: e => {
//       onSlideRelease?.(e);
//       pan.flattenOffset();
//     },
//     onPanResponderEnd: e => {
//       onSlideEnd?.(e);
//       if (
//         pan._value >
//         (sliderWidth - sliderBallWidth - ballPadding) * confirmedPercent
//       ) {
//         Animated.spring(pan, {
//           toValue: sliderWidth - sliderBallWidth - ballPadding * 2,
//           tension: 10, friction: 5,
//           useNativeDriver: IS_NATIVE_DRIVER,
//           restSpeedThreshold: 2000,
//         }).start(({finished}) => {
//           if (finished) {
//             if (disableOnConfirmed) {
//               setDisable(true);
//             }
//             onSlideConfirmed?.(e);
//           }
//         });
//       } else {
//         Animated.timing(pan, {
//           toValue: 0,
//           tension: 10, friction: 5,
//           duration: goToBackDuration,
//           useNativeDriver: IS_NATIVE_DRIVER,
//         }).start(({finished}) => {
//           if (finished) {
//             onSlideNotConfirmed?.(e);
//             if (tipAnimationEnable) {
//               slidePressed();
//             }
//           }
//         });
//       }
//     },
//   });

//   const slidePressed = () => {
//     console.log('-----START----')
//     if (!disable) {
//       Animated.sequence([
//         Animated.timing(pan, {
//           toValue: sliderTipDistanceFromLeft,
//           tension: 10, friction: 5,
//           duration: sliderTipDuration,
//           useNativeDriver: IS_NATIVE_DRIVER,
//         }),
//         Animated.timing(pan, {
//           toValue: 0,
//           tension: 10, friction: 5,
//           duration: sliderTipDuration,
//           useNativeDriver: IS_NATIVE_DRIVER,
//         }),
//       ]).start();
//     }
//   };

//   const slideOpacity = pan.interpolate({
//     inputRange: [
//       0,
//       (sliderWidth - sliderBallWidth - ballPadding) * confirmedPercent,
//     ],
//     outputRange: [1, 0],
//   });

//   const checkOpacity = pan.interpolate({
//     inputRange: [
//       (sliderWidth - sliderBallWidth - ballPadding) * confirmedPercent,
//       sliderWidth - sliderBallWidth - ballPadding,
//     ],
//     outputRange: [0, 1],
//   });

//   useEffect(() => {
//     if (
//       !!unconfimredTipText.length &&
//       !!confirmedTipText.length &&
//       tipTextSlideAnimEnable
//     ) {
//       Animated.loop(
//         Animated.timing(textAnim, {
//           toValue: sliderWidth - sliderBallWidth,
//           duration: 4000,
//           useNativeDriver: IS_NATIVE_DRIVER,
//         }),
//       ).start();
//     }
//   }, [unconfimredTipText, confirmedTipText, tipTextSlideAnimEnable]);

//   useEffect(() => {
//     if (state) {
//       Animated.spring(pan, {
//         toValue: sliderWidth - sliderBallWidth - ballPadding * 2,
//         useNativeDriver: IS_NATIVE_DRIVER,
//         restSpeedThreshold: 4000,
//       }).start(({finished}) => {
//         if (finished) {
//           if (disableOnConfirmed) {
//             setDisable(true);
//           }
//           onSlideConfirmed?.();
//         }
//       });
//     } else {
//       Animated.timing(pan, {
//         toValue: 0,
//         duration: goToBackDuration,
//         useNativeDriver: IS_NATIVE_DRIVER,
//       }).start(({finished}) => {
//         if (finished) {
//           onSlideNotConfirmed?.();
//           if (tipAnimationEnable) {
//             slidePressed();
//           }
//         }
//       });
//     }
//   }, [state]);

//   return (
//     <View
//       style={[
//         {backgroundColor: defaultColor, marginHorizontal: 0},
//         sliderStyle || null,
//         {paddingLeft: ballPadding, paddingRight: ballPadding},
//       ]}>
//       <Animated.View
//         {...panResponder.panHandlers}
//         style={{zIndex: 1, transform: [{translateX: pan},]}}>
//         {!!sliderButtonComponent ? (
//           sliderButtonComponent
//         ) : (
//           <Pressable
//             style={styles.defaultBall}
//             onPress={() => tipAnimationEnable && slidePressed()}>
//             <Animated.View
//               style={[styles.defaultCevronRight, {opacity: slideOpacity}]}>
//               <Image resizeMode='contain' source={IMAGES.ARROW_RIGHT} style={styles.icon} />
//             </Animated.View>
//             <Animated.View
//               style={[styles.defaultChecked, {opacity: checkOpacity}]}>
//               <Image resizeMode='contain' source={IMAGES.ARROW_RIGHT} style={styles.icon} />
//             </Animated.View>
//           </Pressable>
//         )}
//       </Animated.View>
//       {!!unconfimredTipText.length && !!confirmedTipText.length && (
//         <View style={{ justifyContent: 'center', alignItems: 'center'}}>
//           <Animated.Text
//             style={[
//               styles.tipText,
//               {opacity: slideOpacity},
//               unconfirmedTipTextStyle,
//             ]}>
//             {unconfimredTipText}
//           </Animated.Text>
//           <Animated.Text
//             style={[
//               styles.tipText,
//               {opacity: checkOpacity},
//               confirmedTipTextStyle,
//             ]}>
//             {confirmedTipText}
//           </Animated.Text>
//         </View>
//       )}
//       {/* {tipTextSlideAnimEnable && (
//         <Animated.View
//           style={[
//             styles.textSlideAnimStyle,
//             {
//               transform: [{translateX: textAnim}],
//               backgroundColor: sliderStyle?.backgroundColor || defaultColor,
//             },
//           ]}
//         />
//       )} */}
//     </View>
//   );
// };

// const styles = StyleSheet.create({
//   tipText: {
//     // zIndex: 0,
//     bottom: 12,
//     position: 'absolute',
//     alignSelf: 'center',
//     color: '#FFF',
//     fontFamily: GRAPHIK_FONT.MEDIUM,
//     fontSize: 16
//     // backgroundColor: 'lightblue'
//   },
//   defaultBall: {
//     alignItems: 'center',
//     justifyContent: 'center',
//     height: 44,
//     width: 65,
//     marginLeft: 3,
//     marginTop: 3,
//     borderRadius: 25,
//     backgroundColor: 'white',
//     zIndex: 1,
//   },
//   defaultCevronRight: {
//     position: 'absolute',
//     backgroundColor: 'transparent',
//   },
//   defaultChecked: {
//     position: 'absolute',
//     backgroundColor: 'transparent'
//   },
//   textSlideAnimStyle: {
//     height: '100%',
//     opacity: 0.5,
//     width: 10,
//     position: 'absolute',
//   },
//   icon: {
//     width: 20,
//     height: 20,
//   },
// });

// export default SlideButton;
